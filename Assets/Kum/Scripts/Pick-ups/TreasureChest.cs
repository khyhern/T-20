...
public class TreasureChest : MonoBehaviour
{
    private void OnTriggerEnter2D(Collider2D col)
    {
        ...
        Player_Inventory p = col.GetComponent<Player_Inventory>();
        if (p)
        {
    ...
    public void OpenTreasureChest(Player_Inventory inventory, bool isHigherTier)
    {

        // Loop through every weapon to check whether it can evolve.
        foreach (Player_Inventory.Slot s in inventory.weaponSlots)
        {
