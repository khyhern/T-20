...
public class LightningRingWeapon : ProjectileWeapon
{
    ...
    ArrayList allSelectedEnemies = new ArrayList();
    ...
    protected override bool Attack(int attackCount = 1)
    {
        ...
        {
           allSelectedEnemies = new ArrayList(FindObjectsByType<Enemy_Stats>(FindObjectsSortMode.None));  // Using ArrayList
            ActivateCooldown();
            currentAttackCount = attackCount;
        }

        // Find an enemy in the map to strike with lightning.
        Enemy_Stats target = PickEnemy();
        if (target)
        {
    ...
    void DamageArea(Vector2 position, float radius, float damage)
    {
        ...
        {
            Enemy_Stats es = t.GetComponent<Enemy_Stats>();
