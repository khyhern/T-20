...
public class Projectile : WeaponEffect
{
    ...
    public virtual void AcquireAutoAimFacing()
    {
        ...
        Enemy_Stats[] targets = FindObjectsOfType<Enemy_Stats>(); 

        // Select a random enemy (if there is at least 1).
        // Otherwise, pick a random angle.
        if (targets.Length > 0)
        {
            Enemy_Stats selectedTarget = targets[Random.Range(0, targets.Length)];
    ...
    {
        Enemy_Stats es = other.GetComponent<Enemy_Stats>();
